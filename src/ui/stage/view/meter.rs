...
use crate::PROFONT;
use crate::ui::stage::meter::{Cell, Field, Plan, Row};
use eframe::egui;
use egui::Color32;

/// Cache glyph geometry independently of the continuously changing fade.
/// PLACEHOLDER is replaced by the paint colour, so smooth alpha does not
/// produce a distinct layout job for every row on every frame.
trait MeterText {
    fn meter_text(
        &self,
        pos: egui::Pos2,
...
impl MeterText for egui::Painter {
    fn meter_text(
        &self,
        pos: egui::Pos2,
        anchor: egui::Align2,
        ...
        let galley = self.layout_no_wrap(text.to_string(), font, Color32::PLACEHOLDER);
        let rect = anchor.anchor_size(pos, galley.size());
        self.galley(rect.min, galley, colour);
        rect
    }
}
...
mod tests {
    ...
    #[test]
    fn meter_fades_share_text_geometry_without_quantizing_colour() {
        let ctx = egui::Context::default();
        let colours = [
            Color32::from_rgba_unmultiplied(70, 180, 200, 37),
            Color32::from_rgba_unmultiplied(70, 180, 200, 181),
        ];
        let mut output = ctx.run_ui(egui::RawInput::default(), |ui| {
            for (i, colour) in colours.into_iter().enumerate() {
                ui.painter().meter_text(
                    egui::pos2(10.0, 20.0 + i as f32 * 20.0),
        ...
        assert!(std::sync::Arc::ptr_eq(&texts[0].galley, &texts[1].galley));
        assert_eq!(texts[0].fallback_color, colours[0]);
        assert_eq!(texts[1].fallback_color, colours[1]);
...
impl super::super::Stage {
    ...
    fn draw_meter_body(
        ...
        let derived = self.meter_rows_visible(now - above, rows, &lay.visible_tracks);
        ...
        for (at, row) in derived.iter().enumerate() {
            ...
            for (index, &(track, field)) in plan.flat.iter().enumerate() {
                ...
                let drum = plan.columns.get(track).is_some_and(|column| column.drum);
                draw_field(&clip, at, font, ch, cell, field, w, lit, &c, drum);
            }
        }
        ...
        self.draw_meter_readout(
            painter,
            readout,
            font,
            ch,
            plan,
            derived.get(above as usize),
        );
    }
    ...
    fn draw_meter_heads(
        ...
        for (index, &(track, field)) in plan.flat.iter().enumerate() {
                ...
                if focused == Some(track) {
                    painter.rect_filled(
                        egui::Rect::from_min_max(
                            ...
                            egui::pos2(x + lay.track_width(track), rect.max.y - 14.0),
                        ),
                        2.0,
                        alpha(c.select, 70.0),
                    );
            ...
            let head = column.heads.get(index - plan.start_of(track));
            let word = match field {
                Field::Add => match self.meter_add_param_from_plan(track, plan) {
...
struct Lay {
    x: Vec<Option<f32>>,
    width: Vec<f32>,
    track_widths: Vec<f32>,
    visible_tracks: Vec<bool>,
}
...
impl Lay {
    ...
    fn of(plan: &Plan, ch: f32, room: f32, cursor: usize) -> Self {
        ...
        let mut track_widths = vec![0.0; plan.columns.len()];
        for &(track, field) in &plan.flat {
            ...
            width.push(w);
            track_widths[track] += w + Self::FIELD_GAP;
            x += w + Self::FIELD_GAP;
        }
        ...
        let x: Vec<Option<f32>> = raw
            ...
            .collect();
        let mut visible_tracks = vec![false; plan.columns.len()];
        for (&(track, _), x) in plan.flat.iter().zip(&x) {
            visible_tracks[track] |= x.is_some();
        }
        Self {
            x,
            width,
    ...
    fn track_width(&self, track: usize) -> f32 {
        self.track_widths.get(track).copied().unwrap_or(0.0)
    }
}
...
fn draw_field(
    ...
    match field {
        Field::Note => {
            ...
            painter.meter_text(at, egui::Align2::LEFT_CENTER, &text, font.clone(), colour);
