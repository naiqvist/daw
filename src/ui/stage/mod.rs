...
const CLIP_HELP_EXAMPLES: [(&str, &str); 9] = [
    ("SHIFT ARROWS", "extend selection"),
    ("CTRL P", "lock notes / cursor"),
    ("12 W ↑", "note octave up"),
    ("12 W ↓", "note octave down"),
    ("12 SHIFT W ↑", "stack octave up"),
    ("12 SHIFT W ↓", "stack octave down"),
    ("50 C", "chance of 50%"),
    ("E ↑/↓", "euclid: hits ±"),
    ("E ←/→", "euclid: turn"),
];
    ...
    pub fn apply(&mut self, intent: StageIntent) -> ApplyOutcome {
        ...
        let result = match intent {
            ...
            StageIntent::Rewind => {
                ...
                self.transport.rewind();
                // Rewind is an idempotent absolute navigation request. Only
                // a position change needs an audio discontinuity.
    ...
    fn every_dispatched_key_changes_state_or_produces_a_refusal() {
        for scope in keymap::ScopeContext::ALL {
            for (modifiers, key) in keymap::bound_chords() {
                ...
                let before = (
                    stage.focus.clone(),
                    (stage.transport, stage.seeks, stage.palette.is_open()),
                    stage.help,
                    stage.browser.clone(),
                    stage.mixing,
                    stage.playing.clone(),
                ...
                match stage.handle_key(modifiers, key) {
                    Some(ApplyOutcome::Changed) => {
                        ...
                        if _intent == StageIntent::Rewind && before.1.0.tick() == 0 {
                            // Home at zero intentionally succeeds without
                            // re-seeking the engine or mutating any fact.
                            assert_eq!(after, before, "idempotent Home mutated state");
                        } else {
                            assert_ne!(after, before, "{scope:?} + {key:?} lied about changing");
                        }
