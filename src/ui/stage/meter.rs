...
//!
//! Nothing here edits, and nothing here is remembered. Every cell is
//! read out of the document at the moment it is drawn, so there is no
//! second copy of the song to reconcile — which is what will let the
//! cells become editable later without the section growing an authority
//! of its own. The one thing the core does keep is the ring of answered
//! intents, because a stream of events has nowhere else to live.
//!
...
impl super::Stage {
    ...
    fn meter_track_fields(&self, pattern: Option<&Pattern>, kind: DeviceKind) -> Vec<Field> {
        ...
        for step in 0..steps.max(1) {
            for lock in &pattern.trig(step).locks {
                if !seen.contains(&(lock.device, lock.param)) {
    ...
    pub(super) fn meter_add_param(&self, track: usize) -> Option<(u32, &'static str)> {
        ...
        let taken: Vec<u32> = self
            ...
            .unwrap_or_default();
        let free: Vec<(u32, &'static str)> = spec
            .params
            .iter()
            .zip(spec.labels)
            .filter(|(def, _)| !taken.contains(&def.id))
            .map(|(def, label)| (def.id, label.name))
            .collect();
        if free.is_empty() {
            return None;
        }
        Some(free[self.meter.add % free.len()])
    }
                ...
                Row {
                    ...
                    cells: playing
                        ...
                        .map(|held| {
                            ...
                            self.song
                                .pattern(held.id)
                                .map(|pattern| self.meter_cell(pattern, at, held.voice))
                        })
                        .collect(),
                }
            })
    ...
    /// One trig, read out.
    fn meter_cell(&self, pattern: &Pattern, step: usize, voice: Option<DeviceKind>) -> Cell {
        ...
        Cell {
            ...
            locks: self.meter_locks(trig, voice),
            cond: trig.cond,
    ...
    fn meter_locks(&self, trig: &Trig, voice: Option<DeviceKind>) -> Vec<Lock> {
        trig.locks
            .iter()
            .map(|lock: &ParamLock| {
                let kind = match lock.device {
                    None => voice,
                    Some(id) => self.song.device(id).map(|device| device.kind),
                };
                let label = kind.and_then(|kind| label_of(kind, lock.param));
                Lock {
                    device: lock.device,
                    param: lock.param,
    ...
    pub(super) fn apply_meter(
        ...
        match intent {
            ...
            I::AddPrev | I::AddNext => {
                ...
                self.meter.add = next;
                match self.meter_add_param(track) {
        ...
        for (row, at) in self.meter_cells() {
            ...
            match (field, edit) {
                ...
                (Field::Add, Edit::Turn { up: true, .. }) => {
                    let Some((param, _)) = self.meter_add_param(track) else {
                        continue;
                    };
