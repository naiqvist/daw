...
impl FocusLattice {
    ...
    /// Grow or shrink the ROW count, by the same rule the columns follow:
    /// a cursor that fell off the bottom is clamped rather than lost, and
    /// a lattice with no rows keeps none.
    ///
    /// What this is for: the lattice has two contents, and they are not
    /// the same height. The scenes are as many rows as the song has; the
    /// mixer is one row, because a channel is addressed as a whole. The
    /// cursor survives the change instead of being rebuilt, so switching
    /// back does not move the ground.
    pub fn resize_rows(&mut self, rows: usize) {
        self.rows = rows;
        self.cursor = match (self.cols, rows, self.cursor) {
            (0, _, _) | (_, 0, _) => None,
            (_, _, None) => Some((0, 0)),
            (_, _, Some((col, row))) => Some((col, row.min(rows - 1))),
        };
    }
