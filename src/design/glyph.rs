...
//!
//! The shapes live here, in the design layer, so the codex can seal them
//! without reaching up into the frames; `ui::glyph` paints them for the
//! surfaces that read colour from a theme.
//!
...
impl Glyph {
    /// Every family, in the browser's order.
    pub const ALL: [Glyph; 12] = [
        Glyph::Dynamics,
        Glyph::Filter,
        Glyph::Time,
        Glyph::Drive,
    ...
    /// The family's name, as the codex writes it.
    pub const fn name(self) -> &'static str {
        match self {
            Glyph::Dynamics => "DYNAMICS",
            Glyph::Filter => "FILTER",
            Glyph::Time => "TIME",
            Glyph::Drive => "DRIVE",
            Glyph::Modulation => "MODULATION",
            Glyph::Spectral => "SPECTRAL",
            Glyph::Utility => "UTILITY",
            Glyph::Instrument => "INSTRUMENT",
            Glyph::Stack => "STACK",
            Glyph::Saw => "SAW",
            Glyph::Transient => "TRANSIENT",
            Glyph::Sample => "SAMPLE",
        }
    }
}
...
mod tests {
    ...
    fn every_family_has_its_own_name() {
        ...
        for g in ALL {
            assert!(seen.insert(g.name()), "{g:?} shares a name");
        }
    }
}
