...
/// KIT: sixteen bricks under one roof, split across the keyboard from
/// BASE. Six kit-wide rows first, then each pad's block: the brick's
/// twenty-two rows in the brick's own order, then PAN, GROUP, ON.
pub mod kit {
    use super::ParamDef;
    ...
    pub const TIGHT: u32 = 3;
    pub const LEVEL: u32 = 4;
    pub const SOLO: u32 = 5;
    pub const GLOBALS: u32 = 6;
    pub const PADS: usize = 16;
    pub const PER_PAD: u32 = 25;
    /// The pad-local rows past the brick's own.
    pub const PAN: u32 = 22;
    pub const GROUP: u32 = 23;
    pub const ON: u32 = 24;
    pub const GROUPS: u32 = 4;
    pub const LEVEL_MAX: f32 = 2.00;

    pub const GROUP_NAMES: &[&str] = &["none", "A", "B", "C", "D"];
    pub const ON_NAMES: &[&str] = &["mute", "on"];
    pub const SOLO_NAMES: &[&str] = &[
        "all", "p1", "p2", "p3", "p4", "p5", "p6", "p7", "p8", "p9", "p10", "p11", "p12", "p13",
        "p14", "p15", "p16",
    ];

    /// Which pad a row belongs to, and its place in the pad's block;
    /// `None` for the kit-wide rows.
    pub const fn pad_of(id: u32) -> Option<(usize, u32)> {
        if id < GLOBALS {
            return None;
        }
        let local = id - GLOBALS;
        let pad = (local / PER_PAD) as usize;
        if pad >= PADS {
            return None;
        }
        Some((pad, local % PER_PAD))
    }

    /// The row for `sub` of pad `pad`.
    pub const fn pad_param(pad: usize, sub: u32) -> u32 {
        GLOBALS + pad as u32 * PER_PAD + sub
    }
